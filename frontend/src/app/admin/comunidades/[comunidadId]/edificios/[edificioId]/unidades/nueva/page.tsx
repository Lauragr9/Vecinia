...
import type { VecinoMembership } from "@/types/api";
...
export default async function NuevaUnidadPage({
  ...
  const allMemberships = await apiGet<VecinoMembership[]>(`/api/comunidades/${comunidadId}/vecinos`);
  const memberships = allMemberships.filter((m) => m.role === "VECINO" || m.role === "PRESIDENTE");
