...
import { TrendingUp, TrendingDown, PiggyBank, Plus, Receipt } from "lucide-react";
...
export default async function GastosAdminPage({
  ...
  const movimientos = await apiGet<MovimientoContable[]>(`/api/comunidades/${comunidadId}/movimientos`);
        ...
        action={
          <div className="flex gap-2">
            <Button variant="outline" nativeButton={false} render={<Link href={`${basePath}/recibos`} />}>
              <Receipt className="size-4" /> Recibos
            </Button>
